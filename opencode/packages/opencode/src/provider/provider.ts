...
export namespace Provider {
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("opencode")) {
        priority = ["gemini-3-flash", "gemini-2.5-flash", "gpt-5-nano"]
      }
    ...
    if (opencodeProvider) {
      for (const fallback of ["gemini-3-flash", "gpt-5-nano"]) {
        if (opencodeProvider.models[fallback]) {
          return getModel("opencode", fallback)
        }
      }
    }
  ...
  const priority = ["gemini-3-pro", "gemini-3-flash", "gpt-5", "claude-sonnet-4", "big-pickle"]
